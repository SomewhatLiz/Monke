...
puppet func register_player(id: int, new_player_data: String):
...
puppet func pre_start_game():
	...
	rpc_id(1, "register_player", my_name) # 1 is the server's ID
	
	# pretend we dont do timer here
	
	# Load world
	get_node("/root/Main").hide()
	var world = load("res://Maps/" + map + ".tscn").instance()
